...
/**
 * adds a triangle to the current shape using calculated normals, with texture
 */
void addTriangle(
    float x0, float y0, float z0,
    float x1, float y1, float z1,
	...
	texture.push_back(texX0);	
	texture.push_back(texY0);	
	texture.push_back(texX1);	
	texture.push_back(texY1);	
	texture.push_back(texX2);	
	texture.push_back(texY2);	
...
/**
 * adds a triangle to the current shape with normals and texture coords
 */
void addTriangle(
    float x0, float y0, float z0,
    float x1, float y1, float z1,
...
/**
 * gets the texture coordinates for the current shape
 */
float *getTextureCoord( void )
{
    // delete the old point array of we have one
    if( textureArray ) {
        delete [] textureArray;
    ...
    // create and fill a new point array
    textureArray = new float[texture.size()];
    for( int i=0; i < texture.size(); i++ ) {
        textureArray[i] = texture[i];
    }
